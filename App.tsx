...
import { analyzePdfForPpt } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const renderPdfToImages = async (file: File) => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      setLoadingMsg(`PDFを展開中... (${i}/${pdf.numPages} ページ)`);
      const page = await pdf.getPage(i);
      // スケールを1.5に調整（互換性と解像度のバランス）
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      setLoadingMsg("AIがドキュメントを解析して解説を作成中...");
  ...
  const handleDownloadPpt = async () => {
    ...
    try {
      ...
      await createPresentation(analysis);
    } catch (err: any) {
      setAppState({ status: 'error', progress: 0, error: "パワーポイントの生成に失敗しました: " + err.message });
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
      <div className="max-w-5xl mx-auto px-4 py-12">
        <header className="text-center mb-16 animate-in fade-in slide-in-from-top duration-700">
          ...
          <h1 className="text-6xl font-black mb-6 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-600 tracking-tight">
            PDF to AI PowerPoint
          </h1>
          <p className="text-slate-400 text-xl max-w-2xl mx-auto font-medium">
            PDFをアップロードするだけで、AIが各スライドを解析。<br />
            発表用ノート付きのパワーポイントを無料で作成します。
          </p>
        </header>
        ...
        <main className="space-y-6">
          <StepCard number={1} title="PDFをアップロード" active={appState.status === 'idle'} completed={!!pdfFile && appState.status !== 'idle'}>
            <div className="flex flex-col items-center">
              <label className="w-full flex flex-col items-center py-12 bg-slate-800/20 rounded-3xl border-2 border-dashed border-slate-700 cursor-pointer hover:border-cyan-500 hover:bg-slate-800/40 transition-all group">
                ...
                <span className="text-slate-300 font-bold text-lg">{pdfFile ? pdfFile.name : "ファイルを選択して開始"}</span>
          ...
          <StepCard number={2} title="解説の確認と出力" active={['rendering', 'analyzing', 'reviewing', 'completed'].includes(appState.status)} completed={appState.status === 'completed'}>
            {['rendering', 'analyzing'].includes(appState.status) ? (
              ...
              <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 shadow-inner text-center">
                  <h4 className="text-cyan-400 font-black text-2xl mb-2 tracking-tight">{analysis.presentationTitle}</h4>
                  <p className="text-slate-400 text-sm max-w-3xl mx-auto">{analysis.summary}</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[500px] overflow-y-auto pr-4 scrollbar-thin">
                  {analysis.slides.map((slide, idx) => (
                    <div key={idx} className="bg-slate-800/40 p-5 rounded-2xl border border-slate-700/50 flex flex-col gap-4">
                      <div className="aspect-video bg-black rounded-lg overflow-hidden border border-slate-700 shadow-lg relative">
                        ...
                        <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-md text-white text-[10px] px-2 py-1 rounded font-bold">PAGE {idx+1}</div>
                      </div>
                      <div className="space-y-2">
                        <h5 className="font-bold text-slate-100 truncate text-sm">{slide.title}</h5>
                        <p className="text-xs text-slate-400 line-clamp-3 bg-slate-950/50 p-3 rounded-lg border border-slate-800 leading-relaxed">{slide.notes}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex justify-center pt-6">
                  <button onClick={handleDownloadPpt} className="px-12 py-5 bg-gradient-to-r from-cyan-600 to-blue-700 hover:from-cyan-500 hover:to-blue-600 text-white font-black rounded-2xl shadow-2xl transition-all transform hover:scale-105 active:scale-95 flex items-center gap-3">
      ...
      {appState.status === 'error' && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 p-10 rounded-[32px] border border-red-500/30 text-center max-w-lg w-full">
            ...
            <p className="text-slate-400 mb-8">{appState.error}</p>
            <button onClick={() => window.location.reload()} className="w-full py-5 bg-red-600 hover:bg-red-500 text-white font-black rounded-2xl shadow-xl">やり直す</button>
          </div>
        </div>
      )}
    </div>
